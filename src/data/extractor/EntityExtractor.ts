...
export class EntityExtractor {
  ...
  async extract(text: string, docId: string): Promise<Entity[]> {
    ...
    if (this.llmConfig && this.requestUtil) {
      try {
        llmEntities = await this.extractByLLM(text, docId);
      } catch (error) {
    ...
    // 合并实体（去重）
    return this.mergeEntities([...ruleEntities, ...llmEntities]);
  }
  ...
  private mergeEntities(entities: Entity[]): Entity[] {
    ...
    entities.forEach(entity => {
      ...
      if (!uniqueEntities.has(key) || 
          (uniqueEntities.get(key)?.confidence || 0) < (entity.confidence || 0)) {
