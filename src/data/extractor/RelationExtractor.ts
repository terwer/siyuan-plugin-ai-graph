...
export class RelationExtractor {
  ...
  private initDefaultRelations(): void {
    // 关联关系模式
    const associatePatterns = [
      /([^，。；；\s]+)与([^，。；；\s]+)相关/g,
      /([^，。；；\s]+)和([^，。；；\s]+)有关/g,
    ];
    
    // 属于关系模式
    const belongToPatterns = [
      /([^，。；；\s]+)属于([^，。；；\s]+)/g,
      /([^，。；；\s]+)是([^，。；；\s]+)的一部分/g,
    ];
   ...
   * 基于规则的关系抽取
   */
  private async extractByRules(entities: Entity[], text: string, relationPatterns: Map<string, RegExp[]>, docId: string): Promise<Relationship[]> {
  ...
  private mergeRelationships(relationships: Relationship[]): Relationship[] {
    ...
    relationships.forEach(rel => {
      ...
      if (!uniqueRelationships.has(key) || 
          (uniqueRelationships.get(key)?.confidence || 0) < (rel.confidence || 0)) {
