...
export class DatabaseManager {
  ...
  async buildInvertedIndex(docId: string, tokens: Token[]): Promise<void> {
    ...
    try {
      ...
      // 先删除该文档的旧索引
      const deleteIndexEntries = this.db.prepare(
        `DELETE FROM index_entries WHERE doc_id = ?`
      );
      ...
      const getTermIdStmt = this.db.prepare(
        `SELECT term_id FROM inverted_index WHERE term = ?`
      );
  ...
  // 实体相似度相关操作
  async addEntitySimilarity(entityId1: number, entityId2: number, similarityScore: number, calculationMethod: string): Promise<void> {
    if (!this.db) return;
