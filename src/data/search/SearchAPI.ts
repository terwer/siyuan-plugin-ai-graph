...
export class SearchAPI {
   ...
   * 搜索关系
   */
  async searchRelationships(options: {
   ...
   * 计算文档相关性分数
   */
  private async calculateDocumentRelevance(queryTokens: any[], options: SearchOptions): Promise<Array<{ docId: string; score: number; highlights: string[]; }>> {
    try {
      // 文档分数映射
   ...
   * 获取实体的知识图谱
   */
  async getEntityGraph(entityId: string, options: {
    depth?: number;
  ...
  async advancedSearch(query: {
    ...
    tags?: string[];
  }, options: SearchOptions = {}): Promise<SearchResult<Document>[]> {
    try {
      // 基础文本搜索
